...
namespace Yamadev.YamaStream.Editor
{
  ...
  public static class VideoPlayerResolver
  {
    ...
    private static void SetupURLResolveCallback()
    {
      ...
      if (!File.Exists(_youtubeDLPath))
      {
        Debug.LogWarning("Unable to find VRC YouTube-DL or YT-DLP installation, URLs will not be resolved in editor. Test your videos in game.");
        return;
      }
    ...
    {
        var mediaPlayer = avPro.gameObject.AddComponent<MediaPlayer>();
#if UNITY_EDITOR_WIN
        var options = (OptionsWindows)mediaPlayer.GetCurrentPlatformOptions();
        options._audioMode = Windows.AudioOutput.Unity;
#endif
      var vrcAVProVideoScreens = Object.FindObjectsByType<VRCAVProVideoScreen>(FindObjectsInactive.Include, FindObjectsSortMode.None);
      foreach (var screen in vrcAVProVideoScreens)
      {
        if (screen.VideoPlayer == avPro)
        {
          var applyToMesh = screen.gameObject.AddComponent<ApplyToMesh>();
      ...
      var vrcAVProVideoSpeakers = Object.FindObjectsByType<VRCAVProVideoSpeaker>(FindObjectsInactive.Include, FindObjectsSortMode.None);
      foreach (var speaker in vrcAVProVideoSpeakers)
      {
        if (speaker.VideoPlayer == avPro)
        {
          var audioOutput = speaker.gameObject.AddComponent<AudioOutput>();
          audioOutput.Player = mediaPlayer;
        }
      }

      var resolver = new AVProPlayerResolver();
    ...
    private static IEnumerator URLResolveCoroutine(string originalUrl, System.Diagnostics.Process ytdlProcess, UnityEngine.Object videoPlayer, Action<string> urlResolvedCallback, Action<VideoError> errorCallback)
    {
      ...
      {
        Debug.Log($"Successfully resolved URL '{originalUrl}' to '{resolvedURL}'");
